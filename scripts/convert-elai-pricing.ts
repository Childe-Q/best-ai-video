...
/**
 * Convert Elai.io pricing JSON to tools.json pricing_plans format
 */
...
interface ElaiData {
  plans: ElaiPlan[];
  comparison_table: ElaiComparisonTable;
}
...
function extractFeaturesFromComparisonTable(
  comparisonTable: ElaiComparisonTable,
  planName: string
): string[] {
  ...
  for (const row of allRows) {
    ...
    if (planValue === true) {
      ...
      if (
        ...
        !valueStr.toLowerCase().includes('add-on')
      ) {
...
function convertElaiPlan(plan: ElaiPlan, comparisonTable: ElaiComparisonTable): PricingPlan {
  ...
  // For Free plan, ensure we have enough features
  if (plan.name === 'Free' && summaryFeatures.length < 4) {
    const freeFeatures = [
      '1 minute per month',
      '80+ Avatars',
      '75+ Languages',
    ...
    summaryFeatures.push(...freeFeatures.filter(f => !summaryFeatures.some(s => s.toLowerCase().includes(f.toLowerCase()))));
  }
  ...
  for (const feature of comparisonFeatures) {
    ...
    const isDuplicate = Array.from(seen).some(existing => {
      ...
      if (lowerKey.includes('seats') && existingLower.includes('user')) return true;
      if (lowerKey.includes('minutes') && existingLower.includes('minute')) return true;
      return false;
    });
  ...
  // Handle pricing
  let price: PricingPlan['price'] = 'Free'; // Initialize with default value
  ...
  if (plan.pricing.type === 'free') {
    ...
    const monthlyVariant = plan.pricing.price_variants.find(
      v => v.note === undefined || !v.note.includes('annual') && !v.note.includes('Shown alongside')
    );
    const yearlyVariant = plan.pricing.price_variants.find(
      v => v.note && (v.note.includes('annual') || v.note.includes('Shown alongside'))
    );
...
function main() {
  const elaiJsonPath = path.join(process.cwd(), 'src', 'data', 'pricing', 'elai-io.json');
  const toolsJsonPath = path.join(process.cwd(), 'src', 'data', 'tools.json');

  // Read Elai JSON
  if (!fs.existsSync(elaiJsonPath)) {
    console.error(`❌ Elai JSON file not found: ${elaiJsonPath}`);
    console.log('💡 Please create the file with the provided Elai JSON data.');
    process.exit(1);
  }

  const elaiData: ElaiData = JSON.parse(fs.readFileSync(elaiJsonPath, 'utf-8'));

  // Read tools.json
  const toolsData = JSON.parse(fs.readFileSync(toolsJsonPath, 'utf-8'));
  const elaiTool = toolsData.find((t: any) => t.slug === 'elai-io');

  if (!elaiTool) {
    console.error('❌ Elai-io tool not found in tools.json');
    process.exit(1);
  }

  // Convert plans
  console.log('🔄 Converting Elai.io pricing plans...\n');
  const pricingPlans = elaiData.plans.map(plan => convertElaiPlan(plan, elaiData.comparison_table));

  // Update tools.json
  elaiTool.pricing_plans = pricingPlans;

  // Write back
  fs.writeFileSync(toolsJsonPath, JSON.stringify(toolsData, null, 2), 'utf-8');

  console.log('✅ Successfully updated tools.json with Elai.io pricing plans:\n');
  ...
  console.log('📝 Run validation: pnpm validate:tools -- --slugs elai-io');
}
