...
export default function AlternativesLongformTemplate({ data }: AlternativesLongformTemplateProps) {
  ...
  const isToolAlternativesPage = data.pageKind === 'tool';
  ...
  const tryNowClass = isToolAlternativesPage
  ...
  const pageContainerClass = isToolAlternativesPage
    ? 'max-w-[1240px] mx-auto px-4 md:px-8 py-12 md:py-16'
    : 'max-w-[880px] mx-auto px-4 md:px-6 py-10 md:py-14';
  const articleContainerClass = isToolAlternativesPage
    ? 'max-w-[780px] mx-auto'
    : 'max-w-[740px] mx-auto';
  const sectionHeadingClass = isToolAlternativesPage
  ...
  const deepDiveSubheadingClass = isToolAlternativesPage
  ...
  const deepDiveListClass = isToolAlternativesPage
  ...
  const wideImageWrapperClass = 'relative left-1/2 -translate-x-1/2 w-[min(1200px,calc(100vw-3rem))]';

  const renderToolCover = (tool: AlternativesTemplateData['deepDives'][number]) => {
    if (tool.image) {
      return (
        <div className={isToolAlternativesPage ? wideImageWrapperClass : 'w-full'}>
          <div className={isToolAlternativesPage ? 'rounded-[24px] border border-slate-200/90 bg-white p-2 shadow-[0_10px_30px_rgba(15,23,42,0.09)]' : ''}>
            <img
              src={tool.image}
              alt={`${tool.toolName} product image`}
              className={isToolAlternativesPage
    ...
    return (
      <div className={isToolAlternativesPage ? wideImageWrapperClass : 'w-full'}>
        <div className={isToolAlternativesPage
  ...
  const renderDeepDive = (
    ...
    return (
        ...
        >
        <h2 className={isToolAlternativesPage
        ...
        <figure className={isToolAlternativesPage ? 'mt-7 space-y-3.5' : 'mt-5 space-y-2.5'}>
          {renderToolCover(tool)}
          <figcaption className={isToolAlternativesPage ? 'text-xs text-slate-500/90 italic pl-1' : 'text-xs text-slate-500'}>
            Images are screenshots provided by our team.
          </figcaption>
        </figure>

        <div className={isToolAlternativesPage ? 'mt-10 space-y-9' : 'mt-7 space-y-7'}>
          <section>
            <p className={isToolAlternativesPage ? 'text-[18px] leading-[1.9] text-slate-700' : 'text-[18px] leading-8 text-slate-700'}>
          ...
          <section>
            ...
            <p className={isToolAlternativesPage ? 'mt-2 text-[17px] leading-[1.85] text-slate-700' : 'mt-2 text-[17px] leading-8 text-slate-700'}>
              {tool.pricingStarting}
            </p>
          </section>
        </div>

        <div className={isToolAlternativesPage ? 'mt-10 flex flex-wrap items-center gap-3.5' : 'mt-7 flex flex-wrap items-center gap-3'}>
          <Link
            href={tool.ctaHref}
      ...
      <div className={pageContainerClass}>
        <article className={articleContainerClass}>
          <header id="article-header" className={isToolAlternativesPage ? 'pb-12' : 'pb-10'}>
            ...
            <p className={isToolAlternativesPage ? 'mt-7 text-[17px] leading-[1.85] text-slate-700' : 'mt-6 text-[17px] leading-8 text-slate-700'}>
          ...
          <section id="tldr" className={isToolAlternativesPage ? 'scroll-mt-28 mt-16' : 'scroll-mt-28 mt-14'}>
            <h2 className={sectionHeadingClass}>TL;DR</h2>
            <ul className={isToolAlternativesPage ? 'mt-6 space-y-3.5 pl-6 list-disc list-outside text-[17px] leading-[1.85] text-slate-700 marker:text-slate-400' : 'mt-5 space-y-3 text-[17px] leading-8 text-slate-700 list-disc list-inside marker:text-slate-400'}>
          ...
          <section id="why-consider" className={isToolAlternativesPage ? 'scroll-mt-28 mt-16' : 'scroll-mt-28 mt-14'}>
            <h2 className={sectionHeadingClass}>Why consider an alternative?</h2>
            <ol className={isToolAlternativesPage ? 'mt-7 space-y-9' : 'mt-6 space-y-8'}>
              {whyConsiderReasons.map((item, index) => (
                <li key={item.title}>
                  <h3 className={isToolAlternativesPage ? 'text-[1.32rem] font-bold text-slate-900 leading-snug' : 'text-xl font-bold text-slate-900'}>
                    {index + 1}. {item.title}
                  </h3>
                  <div className={isToolAlternativesPage ? 'mt-3 space-y-4 text-[17px] leading-[1.85] text-slate-700' : 'mt-2 space-y-3 text-[17px] leading-8 text-slate-700'}>
          ...
          <section id="top-six" className={isToolAlternativesPage ? 'scroll-mt-28 mt-16' : 'scroll-mt-28 mt-14'}>
            <h2 className={sectionHeadingClass}>
              The {topPicksLimit} most popular {baseToolName} alternatives
            </h2>
            <div className={isToolAlternativesPage ? 'mt-4' : 'mt-2'}>
              {visibleDeepDives.map((tool, index) => renderDeepDive(tool, 'primary', index, true))}
            </div>
          </section>

          <section id="at-a-glance" className={isToolAlternativesPage ? 'scroll-mt-28 mt-16' : 'scroll-mt-28 mt-14'}>
          ...
          <section id="faq" className={isToolAlternativesPage ? 'scroll-mt-28 mt-16' : 'scroll-mt-28 mt-14'}>
